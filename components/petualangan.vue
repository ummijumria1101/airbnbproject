<template>
    <div>
        <div>
            <p class="mt-[88px] ml-[80px] text-xl font-semibold">Temukan Petualangan Airbnb</p>
             <p class="mt-[8px] ml-[80px] text-base text-gray-400 mb-[24px]">Nikmati perjalanan setiap hari dengan santai bersama ahli lokal. Kegiatan, makan, dan akomodasi sudah termasuk. Bergabunglah sekarang!</p>
        </div>
        <div class="grid lg:grid-cols-6 md:grid-cols-3 sm:grid-cols-2 xs:grid-cols-1 ml-[80px] mr-[80px] gap-[8px] overflow-hidden">
            <div class="">
                <img src="/img/pet1.png" class="w-[188px] h-[240px] rounded-md">
                <p class="text-sm text-gray-500">Cayman Islands</p>
                <p class="text-base font-semibold">2 Nights PACKAGE All <br> Inclusive</p>
                <p class="text-sm text-gray-500">Rp 10.000.000</p>
                <div class="flex">
                    <p class="text-cyan-600">5.0 </p>
                    <img src="/img/icons8-star-50.png" class="w-[15px] h-[15px] items-center pl-1 mt-[5px]">
                </div>
                
            </div>
            <div class="">
                <img src="/img/pet2.png" class="w-[188px] h-[240px] rounded-md">
                <p class="text-sm text-gray-500">Cayman Islands</p>
                <p class="text-base font-semibold">2 Nights PACKAGE All <br> Inclusive</p>
                <p class="text-sm text-gray-500">Rp 10.000.000</p>
                <div class="flex">
                    <p class="text-cyan-600">5.0 </p>
                    <img src="/img/icons8-star-50.png" class="w-[15px] h-[15px] items-center pl-1 mt-[5px]">
                </div>
                
            </div>
            <div>
                <img src="/img/pet3.png" class="w-[188px] h-[240px] rounded-md">
                <p class="text-sm text-gray-500">Cayman Islands</p>
                <p class="text-base font-semibold">2 Nights PACKAGE All <br> Inclusive</p>
                <p class="text-sm text-gray-500">Rp 10.000.000</p>
                <div class="flex">
                    <p class="text-cyan-600">5.0 </p>
                    <img src="/img/icons8-star-50.png" class="w-[15px] h-[15px] items-center pl-1 mt-[5px]">
                </div>
                
            </div>
            <div>
                <img src="/img/pet4.png" class="w-[188px] h-[240px] rounded-md">
                <p class="text-sm text-gray-500">Cayman Islands</p>
                <p class="text-base font-semibold">2 Nights PACKAGE All <br> Inclusive</p>
                <p class="text-sm text-gray-500">Rp 10.000.000</p>
                <div class="flex">
                    <p class="text-cyan-600">5.0 </p>
                    <img src="/img/icons8-star-50.png" class="w-[15px] h-[15px] items-center pl-1 mt-[5px]">
                </div>
                
            </div>
            <div>
                <img src="/img/pet5.png" class="w-[188px] h-[240px] rounded-md">
                <p class="text-sm text-gray-500">Cayman Islands</p>
                <p class="text-base font-semibold">2 Nights PACKAGE All <br> Inclusive</p>
                <p class="text-sm text-gray-500">Rp 10.000.000</p>
                <div class="flex">
                    <p class="text-cyan-600">5.0 </p>
                    <img src="/img/icons8-star-50.png" class="w-[15px] h-[15px] items-center pl-1 mt-[5px]">
                </div>
                
            </div>
            <div>
                <img src="/img/pet6.png" class="w-[188px] h-[240px] rounded-md">
                <p class="text-sm text-gray-500">Cayman Islands</p>
                <p class="text-base font-semibold">2 Nights PACKAGE All <br> Inclusive</p>
                <p class="text-sm text-gray-500">Rp 10.000.000</p>
                <div class="flex">
                    <p class="text-cyan-600">5.0 </p>
                    <img src="/img/icons8-star-50.png" class="w-[15px] h-[15px] items-center pl-1 mt-[5px]">
                </div>
                
            </div>

        </div>
    </div>
</template>

<script setup>

</script>

<style lang="scss" scoped>

</style>